export default function onSubmit(renderer, app, event) {
    ...
    .catch((res) => {
      ...
      switch (res.cause) {
        ...
        case 'status': {
          ...
          if (status === 401) { errorDiv.innerHTML = t('WrongUsernameAndPassword'); }
          else if (status === 429) { errorDiv.innerHTML = t('TooManyRequestsError'); }
          else {
        ...
        default: {
          ...
          debug('onSubmit.js: unknown error');
          debug(`error: ${JSON.stringify(res, Object.getOwnPropertyNames(res))}`);
          break;
        }
      }
    });
