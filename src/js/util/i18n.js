...
export default function i18n(app) {
  ...
  const detectBrowserLocale = () => {
    let locale = browser.i18n.getUILanguage().replace(/-/g, '_');
    if (this.languageMap.has(locale)) { return locale; }
  ...
  const detectLocale = () => {
    ...
    if (acceptedLocales.includes(locale)) { return locale; }
    return undefined;
  };
  ...
  let worker = null;
  this.worker = () => { return worker; };
  this.defaultLocale = 'en';
  this.locale = detectLocale() || this.defaultLocale;
  this.domain = () => { return this.domainMap.get(this.locale) || this.domainMap.get('en'); };

  this.t = (key, variables = {}) => {
    let message = translations.get(key) || browser.i18n.getMessage(key);
  ...
  this.changeLocale = (locale) => {
    const { icon } = app.util;
    const targetURL = browser.extension.getURL(`/_locales/${locale}/messages.json`);

    if (!acceptedLocales.includes(locale)) {
      return new Promise((_, reject) => { reject(new Error()); });
    }

    worker = http.get(targetURL)
      .then(async (res) => {
